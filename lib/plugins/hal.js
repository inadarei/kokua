...
class HalPlugin {
  ...
  handleBody() {
    let curr;
    const paths = traverse(this.doc).paths(); // unfortunately _.at() doesn't do the same thing :(
    for (const path of paths) {
      //log.info("path ", path);
      //curr = _.cloneDeep(traverse(this.doc).get(path));
      curr = _.cloneDeep(_.get(this.doc, path));
      if (typeof curr === 'object') {
        if (!Array.isArray(curr) && !hutil.skipProps.includes(path[0])) {
          if (curr && (curr["h:ref"] || curr["h:link"])) {
  ...
  copyBodyProps() {
    ...
    for (const prop in body) {
      if (!hutil.skipProps.includes(prop)) {
